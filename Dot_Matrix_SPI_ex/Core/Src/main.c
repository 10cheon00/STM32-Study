...
int main(void) {
     ...
     * ------------------------------------------------------------
     * 1. 파워가 연결되면 모든 레지스터(위에 적은 것들)가 초기화 된다.
     * 2. 먼저 셧다운 모드로 설정된다.
     * 3. 1개의 digit만 스캔하는 모드로 설정된다.
     * 4. 밝기 레지스터도 최소로 설정된다.
     * ------------------------------------------------------------
     * When no-decode is selected, data bits D7–D0 correspond to
     *  the segment lines of the MAX7219/MAX7221.
     * Table 6 shows the one-to-one pairing of each data bit
     * to the appropriate segment line.
     * ------------------------------------------------------------
     * 문서가 8-digit LED에 맞추어 작성되어 있다.
     * Table.5를 참고하면, D7~D0과 SegA~SegG핀의 값에 따라 7-segment의
     *  어떤 LED가 켜지는지 알 수 있다. 본래 기능대로라면 Digit Driver에서
     *  LED가 나타낼 정수값이, Segment Driver에서는 그 정수값을 표현하기
     *  위해 어떤 Segment가 켜지는지 출력된다.
     * 8x8 매트릭스를 위해서는 no-decode 모드로 설정해야한다.
     * no-decode 모드는 Digit Driver와 Segment Driver에 출력될 값이
     *  DIN(SPI의 MOSI)으로 전달된 데이터값이다. 디코드 되지 않고 곧바로
     *  출력된다.
     * 왜 Digit Driver가 매트릭스의 행이고 Segment Driver가
     *  매트릭스의 열인지 모르겠다. 매트릭스의 데이터 시트에 내용이 부족해
     *  이유를 정확히 알지 못했다.
     * ------------------------------------------------------------
     */
    matrix_init(&hspi1);
