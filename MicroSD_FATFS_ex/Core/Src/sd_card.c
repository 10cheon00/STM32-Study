...
static uint8_t SDCARD_ReadR1() {
    ...
    uint8_t tx = 0xFF;
    for (;;) {
        HAL_SPI_TransmitReceive(&SDCARD_SPI_PORT, &tx, &r1, sizeof(r1),
                                HAL_MAX_DELAY);
...
static int SDCARD_WaitDataToken(uint8_t token) {
    ...
    uint8_t tx = 0xFF;
    for (;;) {
        HAL_SPI_TransmitReceive(&SDCARD_SPI_PORT, &tx, &fb, sizeof(fb),
                                HAL_MAX_DELAY);
        if (fb == token)
            break;
...
int SDCARD_Init() {
    ...
    */
...

    {
        ...
        HAL_SPI_Transmit(&SDCARD_SPI_PORT, (uint8_t *)cmd, sizeof(cmd),
                         HAL_MAX_DELAY);
    }
...
int SDCARD_GetBlocksNumber(uint32_t *num) {
    ...
    // Blocks number: CARD_SIZE/BLOCK_SIZE = (C_SIZE+1)*(1<<19) / (1<<9) =
    // (C_SIZE+1)*(1<<10)
    tmp = (tmp + 1) << 10;
    *num = tmp;
...
int SDCARD_WriteSingleBlock(uint32_t blockNum, const uint8_t *buff) {
    ...
    HAL_SPI_Transmit(&SDCARD_SPI_PORT, &dataToken, sizeof(dataToken),
                     HAL_MAX_DELAY);
...
int SDCARD_WriteEnd() {
    ...
    HAL_SPI_Transmit(&SDCARD_SPI_PORT, &stopTran, sizeof(stopTran),
                     HAL_MAX_DELAY);
